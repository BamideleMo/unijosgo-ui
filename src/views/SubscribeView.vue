...
<template>
  <SuperMeta :title="title" name="description" content="Join a community of fun loving Jossites." />
  
  <div class="grid grid-cols-1 sm:grid-cols-2 sm:gap-10 text-black">
    <div class="">
      <h1 class="h1 font-semibold text-lg">Subscribe</h1>
      <div class="mt-6 space-y-4 text-sm">
        ...
        <div class="">
          ...
          <input type="text" v-model="formData.name" @blur="v$.name.$touch" placeholder="What should we call you?"
            class="w-full placeholder:text-red-100 mt-1 rounded-md outline-none px-3 py-1 h-10 border-2 bg-transparent border-red-500" />
          <div class="text-right text-red-500 uppercase animate-pulse mt-2 text-xs font-normal space-x-1"
            v-if="v$.name.$error">
    ...
    <div class="space-y-4 text-sm">
      <div class="mt-6 sm:mt-16 pt-2 font-semibold">Already subscribed? <RouterLink class="text-red-600 hover:opacity-60" to="/sign-in">Sign In</RouterLink> instead.</div>
      <p class="">
          By clicking the submit button you agree to recieve an SMS or a WhatsApp message to verify your
          phone number.
        </p>
        <p>UniJosGist will never share your information with anyone.</p>
    </div>
  </div>
</template>
